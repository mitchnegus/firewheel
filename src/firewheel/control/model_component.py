...
class _BinaryFile(ABC):
    ...
    def _determine_file_store_status(self):
        # Exit immediately if the file has not yet been uploaded
        if (store_timestamp := self.file_store.get_file_upload_date(self.name)) is None:
            return _FileStoreStatus.MISSING
        # The file exists in the store, so we must compare the two files
        ...
        elif store_timestamp < self.modification_timestamp:
            return _FileStoreStatus.OUTDATED
        return _FileStoreStatus.UKNOWN
...
class ModelComponent:
    ...
    def __init__(
        ...
        **If both are specified, name must match the MANIFEST at path.**
        ...
        self.vm_resource_store = vm_resource_store or VmResourceStore()
        ...
        self.overall_cache_progress = Progress(
            ...
            TextColumn(
                "[yellow]Populating/refreshing the caches for MC "
    ...
    def upload_files(self):
        ...
        """
        files = self._collect_vm_resources() + self._collect_images()
        file_uploads = [file for file in files if file.requires_upload]
        with Live(self.cache_progress_group):
            overall_task_id = self.overall_cache_progress.add_task(
    ...
    def _upload_large_file(self, file):
        ...
        update_cache_task_id = self.large_file_cache_progress.add_task(
            description=f"{action} large file: `{file.path}`"
        )
        file.upload()
    ...
    def _collect_vm_resources(self):
        ...
        """
        if (vm_resource_list := self.manifest.get("vm_resources")) is None:
            return
        elif not isinstance(vm_resource_list, list):
